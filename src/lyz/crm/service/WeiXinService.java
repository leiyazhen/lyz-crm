...
import lyz.crm.exception.WeiXinException;
import lyz.crm.pojo.User;
import lyz.crm.utils.HttpUtil;
...
public class WeiXinService {
	...
	private LoadingCache<String, String> accessTokenCache = CacheBuilder
			...
			.build(new CacheLoader<String, String>() {
				...
				public String load(String s) throws Exception {
					...
					String url = MessageFormat.format(ACCESS_TOKEN_URL, corpid,
							secret);
					...
					if (map == null || map.containsKey("errcode")) {
						throw new WeiXinException("获取AccessToken异常："
								+ errorCodeMap.get(map.get("errcode")));
					}
	...
	public void bindUserWeixin(User user, String[] roles) {
		...
		try {
			...
			Map<String, Object> resultMap = gson
					.fromJson(result, HashMap.class);
			String errorCode = Integer.valueOf(
					Double.valueOf(resultMap.get("errcode").toString())
							.intValue()).toString();
			if (!errorCode.equals("0")) {
				throw new WeiXinException("添加通讯录成员异常:"
						+ errorCodeMap.get(errorCode));
			}
	...
	public void delUser(String userid) {
		...
		try {
			...
			String url = MessageFormat
					.format(DEL_USER_URL, accessToken, userid);
			String result = HttpUtil.getString(url);

			Map<String, Object> resultMap = new Gson().fromJson(result,
					HashMap.class);
			String errorCode = Integer.valueOf(
					Double.valueOf(resultMap.get("errcode").toString())
							.intValue()).toString();
			if (!errorCode.equals("0")) {
				throw new WeiXinException("从通讯录中删除成员异常:"
						+ errorCodeMap.get(errorCode));
			}
	...
	public String useCodeGetUserId(String code) {
		try {
			String accessToken = accessTokenCache.get("");
			String url = MessageFormat.format(USE_CODE_GET_USERID, accessToken,
					code);
			...
			if (resultMap == null || resultMap.containsKey("errcode")) {
				String errorCode = Integer.valueOf(
						Double.valueOf(resultMap.get("errcode").toString())
								.intValue()).toString();
				throw new WeiXinException("通过Code获取UserID异常:"
						+ errorCodeMap.get(errorCode));
			}
			return result;
		} catch (ExecutionException e) {
