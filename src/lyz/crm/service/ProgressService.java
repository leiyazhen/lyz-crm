...
import lyz.crm.mapper.CustomerMapper;
import lyz.crm.mapper.ProgressFileMapper;
import lyz.crm.mapper.ProgressMapper;
import lyz.crm.pojo.Customer;
import lyz.crm.pojo.Progress;
import lyz.crm.pojo.ProgressFile;
import lyz.crm.utils.Page;
import lyz.crm.utils.QiniuUtil;
...
public class ProgressService {
	 ...
	 * @return
	 */
	public Page<Progress> findProgressByPageAndParam(String userid,
			String progress, String date, String context, String p) {
		...
		if (file != null && file.length > 0) {
			for (MultipartFile multipartFile : file) {
				if (!multipartFile.isEmpty()) {
					String key = null;
					try {
						key = qiniuUtil.uploadFile(multipartFile
								.getInputStream());
					} catch (IOException e) {
